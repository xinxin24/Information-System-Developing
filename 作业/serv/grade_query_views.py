...
async def view_list_grade(request):
    ...
    return render_html(request, 'grade_query_list.html',
                       students=students,
                       courses=courses,
                       items=items)


@web_routes.get('/grade-query/edit/{stu_sn}/{cou_sn}')
def view_grade_editor(request):
    ...
    return render_html(request, "grade_query_edit.html",
                       stu_sn=stu_sn,
                       cou_sn=cou_sn,
                       term=term,
                       grade=record.grade)
...
@web_routes.get("/grade-query/delete/{stu_sn}/{cou_sn}")
